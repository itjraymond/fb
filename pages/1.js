if ( ! Detector.webgl ) Detector.addGetWebGLMessage();
        ...
        function init() {
            ...
            camera = new THREE.PerspectiveCamera( 22, window.innerWidth / window.innerHeight, 5, 766 );
			...
			var dlight = new THREE.DirectionalLight(0xffe900, .15, 0, 0, 1);
			scene.add(dlight);

			var dlight = new THREE.DirectionalLight(0x84b9ff, .45);
			scene.add(dlight);

			var directionalLight = new THREE.DirectionalLight( 0x9e9e9e, .41 );
			directionalLight.position.set( 56, -55, 222 );
			...
			var bgMat = new THREE.MeshBasicMaterial( { color: 0x000000, shading: THREE.FlatShading, transparent:true, opacity:.4, vertexColors: THREE.VertexColors } );
			...
			var mat5 = new THREE.MeshPhongMaterial( { vertexColors: THREE.VertexColors, side:THREE.DoubleSide, ambient: 0xff1a1a, envMap: reflectionCube, combine: THREE.MixOperation, reflectivity: 0.1 } );
...
*/
		...
		function render() {
			...
			camera.position.x += ( mouseX - camera.position.x ) * .009;
